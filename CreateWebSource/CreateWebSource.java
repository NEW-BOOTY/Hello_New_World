...
public class CreateWebSource {
      ...
      if (args == null || args.length != 4) {
        ...
        String webSourceXML =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><search:config productVersion=\"11.1.2.0.0\""
                + " xmlns:search=\"http://xmlns.oracle.com/search\">  <search:sources>   "
                + " <search:webSource>      <search:name>web1</search:name>       "
                + " <search:startingUrls>          <search:startingUrl>            <search:url>"
                + webSourceURL
                + "</search:url>"
                + "          </search:startingUrl>"
                + "        </search:startingUrls>"
        ...
        adminPort.createAll("source", webSourceXML, "password", credentials, null, null, "en");

        // 2. Export all sources to show the full definition
        ObjectOutput oo = adminPort.exportAll("source", null, "password", credentials, null, "en");
        System.out.println("Web Source XML = \n" + oo.getObjectXML());

        // 3. Create a source group for the source
        String sourceGroupXML =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><search:config productVersion=\"11.1.2.0.0\""
                + " xmlns:search=\"http://xmlns.oracle.com/search\">  <search:sourceGroups>   "
                + " <search:sourceGroup>      <search:name>Web</search:name>     "
                + " <search:assignedSources>       "
                + " <search:assignedSource>web1</search:assignedSource>     "
                + " </search:assignedSources>    </search:sourceGroup>  </search:sourceGroups>"
                + "</search:config>";

        adminPort.createAll("sourceGroup", sourceGroupXML, null, credentials, null, null, "en");
        ...
        String scheduleXML =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><search:config productVersion=\"11.1.2.0.0\""
                + " xmlns:search=\"http://xmlns.oracle.com/search\">   <search:schedules>     "
                + " <search:schedule>         <search:name>schedule1</search:name>        "
                + " <search:crawlingMode>ACCEPT_ALL</search:crawlingMode>        "
                + " <search:recrawlPolicy>PROCESS_CHANGED</search:recrawlPolicy>        "
                + " <search:frequency>            <search:manual/>         </search:frequency>     "
                + " <search:assignedSources>       "
                + " <search:assignedSource>web1</search:assignedSource>     "
                + " </search:assignedSources>      </search:schedule>   </search:schedules>"
                + "</search:config>";

        adminPort.createAll("schedule", scheduleXML, null, credentials, null, null, "en");
        ...
        adminPort.start("schedule", objectKey, null, null, credentials, null, null, "en");
  ...
  private static AdminPortType getStatelessWebServiceClient(String webServiceURL) throws Exception {
    AdminService adminService =
        new AdminService(
            new URL(webServiceURL), new QName("http://search.oracle.com/Admin", "AdminService"));
